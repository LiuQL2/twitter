...
def calculate_lifecycle():
    write_log(log_file_name='calculate_lifecycle_yang_1.log', log_file_path=os.getcwd(),
...
def get_all_dataFrame(data_path):
    ...
    for file_name in file_name_list:
        index += 1
        write_log(log_file_name='calculate_lifecycle_yang_1.log',log_file_path=os.getcwd().replace('process',''),information=str(index) + ': Reading file to dataFrame:' + file_name + ' is being reading...')
    ...
    write_log(log_file_name='calculate_lifecycle_yang_1.log', log_file_path=os.getcwd(),information='tweet_dataFrame has been built, total number:' + str(len(tweet_dataFrame)))
...
def build_tweet(tweet_dataFrame,path_save_to,file_name_save_to):
    ...
    write_log(log_file_name='calculate_lifecycle_yang_1.log', log_file_path=os.getcwd(),information='Finding root tweet for each tweet')
    column = ['tweet_id', 'tweet_time', 'origin_tweet_id', 'origin_tweet_id']
    file = open(path_save_to + file_name_save_to,'wb')
    error_file = open(path_save_to + file_name_save_to.replace('.csv','_error.csv'),'wb')
    ...
    for index in tweet_dataFrame.index:
        ...
        if count - temp_count >= 10000:
            write_log(log_file_name='calculate_lifecycle_yang_1.log', log_file_path=os.getcwd(),information='Finding root tweet, total_number:'+str(total_number)+',finished_number:'+str(count) + '   Finding root tweet for each tweet')
            temp_count = count
    file.close()
    error_file.close()
